...
1. **REST Session Heartbeat (replaced the 20-min browser refresh, added 2026-08)**: `tokenRefreshScheduler.runSessionHeartbeat()` fires ONE cheap authenticated `GetUserCartDetails` probe every `trigger_pharmarack_refresh_interval_min` (default 20) — the same endpoint the retailer web app itself polls, so it is indistinguishable from normal usage (owner zero-ban-risk requirement). A headless Chrome launches ONLY on proven auth death (probe 401/403 → `executeRefresh()`), when a profile exists with no stored token (legacy capture), or via user-clicked manual re-auth. Do NOT reintroduce a periodic browser refresh loop. The heartbeat is P4-exempt (runs while idle — that IS its purpose); only `bg.pharmarackTokenRefresh` mode `off` suppresses it.
2. **Refresh Mutex**: `executeRefresh()` is single-flight — cron ticks, 401 retries from `fetchPharmarack`/catalog cache, and heartbeats share ONE in-flight browser restore instead of spawning concurrent Chromes. Never call browser-launch paths that bypass this mutex.
3. **Profile Lock Resolution**: Chrome profile lock files (`SingletonLock`, `lockfile`, etc.) are cleaned dynamically before launching Puppeteer to avoid lock crashes. Profile copies (`copyProfileFolder`) are async — never revert to sync `fs.copyFileSync` walking (it blocked the event loop during lock-fallbacks).
4. **Session Cookie Preservation**: When refreshes or cart fallbacks copy the profile directory to a temporary path, the updated session data and rolling cookies must be copied back to the main profile (`data/pharmarack_profile`) on exit, ensuring the primary profile remains authenticated.
5. **Boot Live-Cart Warm-up** (added 2026-08): after the first boot keep-alive settles, the backend proactively loads the live cart via `warmupStartupCart()` (chained on `tokenRefreshScheduler.onFirstRefreshComplete()` + `runSessionHeartbeat('boot')`, with a T+50s fallback) so `startupSyncCoordinator` reflects real sync state instead of waiting for a UI visit — no false "cart sync pending" toast on first boot; genuine session expiry still surfaces the toast truthfully.
