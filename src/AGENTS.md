...
- **Bill Amount line is mandatory on order alerts**: extraction (`extractBillAmount`, shared pattern) covers subject+body AND text attachments (CSV/TXT ≤100 KB). When genuinely undetectable the alert prints `Bill Amount: N/A` — NEVER hide the line, estimate from items, or fabricate a total.
- **Distributor resolution (extractOrderInfo)**: priority 1 = clean-vs-clean sender-email compare against saved distributor profiles (the AI Learning page's mail ID field, compared via `extractCleanEmail` on BOTH sides so display-formatted/multi-address entries resolve); priority 2 = exact display-name match, which LEARNS BACK the sender mail ID onto `distributors.email` (append-only, idempotent — never overwrite an existing address). The legacy hardcoded keyword→name list was REMOVED by owner decision; do not reintroduce per-sender name mappings anywhere.
- Learn-back fires ONLY on exact display-name equality (no fuzzy distributor linking) and persists durably in SQLite — this is the semantic-memory layer that makes daily silent syncs smarter without losing user adjustments.
...
- Implemented as `scripts/watchdog.mjs` (added 2026-08-25; the previously-referenced `self-healing-spec.md` was never committed). Launches the packaged exe (or `npm start` fallback), restarts on exit code 1 with the WorkerSupervisor backoff (>30 s stable run resets; n×3 s delay, cap 5 fast crashes then CRITICAL + stand-down), logs to `<dataDir>/watchdog.log`. `--self-test` verifies the policy without launching the app. Intentional stops (code 0 / operator codes) are never restarted.
- Backup restorability is machine-provable via `scripts/backup-restore-drill.mjs` (`npm run drill`): sandboxed snapshot → service-format .db.gz backup → post-backup writes → restore → integrity/canary/FTS verification. Live DB is opened read-only as source only.
...
- POST|PUT /api/orders/:id/status AND the generic PUT /api/orders/:id (CRM edit modal / status buttons): when status becomes Ready, the handler queues the localized arrival WhatsApp inside the SAME user-clicked request via shared helper enqueueArrivalWhatsApp (also used by /:id/notify-arrival). Idempotent on notified===1; missing phone skips silently; both responses carry whatsapp_queued. This preserves the Strict Manual-Only Patient Messaging Contract: no worker or background job may ever dispatch it.
- Arrival detection still only flips status to Ready/ARRIVED with notified = 0; real match_type/match_confidence are stored in order_overlaps.
- special_orders.phone is DIGITS-ONLY everywhere (bug P2-08, 2026-08-25): the WhatsApp intent pipeline, createSpecialOrderFromShortage, and PUT /api/orders/:id all strip non-digits before writing — chat-id suffixes (@c.us/@lid) must never reach the column. Existing dirty rows were fixed once by `scripts/normalizeSpecialOrderPhones.mjs` (idempotent, --dry-run; also extracts a 10-digit number embedded in the requester name when it contradicts the stored phone). Do not reintroduce raw writes or boot-time auto-migration (owner chose one-shot script).
...
- POST /api/whatsapp/queue/items/:id/resend re-enqueues any sent/failed/pending item as a NEW queue item and force-dispatches it. It relies on enqueue(..., { skipDedupe: true }) because the default same-day number+message dedupe would otherwise silently suppress an identical resend. Source resolution order (bug P2-07 fix, 2026-08-25): (1) whatsapp_send_queue row by id, (2) automation_notifications failure row via id − 900000 (the UI's failure-log cards), (3) explicit {number,message,targetName} body payload for hash-mapped direct rows (id ≥ 800000). Invalid stored numbers fail with an actionable 400 — never silently dropped.
- DELETE /api/messaging/chats/:chatId/messages/:messageId (added 2026-08-25): user-clicked removal of a RECEIVED message from the LOCAL inbox cache only — deletes the whatsapp_messages row plus best-effort cached media (data/inbound_media/<safeId>.jpg via the writer's id sanitization, <appData>/uploads/<msgId>*). NEVER revokes/unsend on WhatsApp itself; the sender's copy is untouched. The CRM chat pane exposes it ONLY for received voice notes (type ptt/audio).
...
- **Background Job Lane (added 2026-08-25)**: `src/utils/backgroundJobLane.ts` `runHeavyJob(name, fn)` is the global serialization lane for HEAVY background jobs — a FIFO promise chain guaranteeing no two heavy jobs ever run concurrently (fixes node-cron "missed execution" pile-ups, first observed 2026-08-25 11:20 IST when the Pharmarack minute-tick collided with other work), plus per-name single-flight: a same-name tick arriving while its job is queued/running is SKIPPED and logged, never double-queued. Lane-routed today: triggerSchedulerService's daily_check / expiry_scan / backup / expiry_returns / doctor_report, server.ts pharmarack_daily_dispatch, pharmarackCatalogCache sync, imageArchiveService cleanup+zip, stockCalculatorWorker periodic recalc. Deliberately NOT lane-routed: tokenRefreshScheduler heartbeat (P4-exempt lightweight credential probe) and whatsappQueueWorker dispatch (messaging latency must not queue behind scans). New heavy background jobs MUST route through `runHeavyJob()`; never await `runHeavyJob` from inside a running job (deadlock). The expiry-return daily tick default moved `'0 9 * * *'` → `'10 9 * * *'` so fresh installs don't stack on daily_check's 09:00.
...
- `WWEBJS_AUTH_DIR` env override (added 2026-08-25, test-safety seam): `whatsappClient.ts` resolves its LocalAuth directory from this env var when set, else `<appDataDir>/.wwebjs_auth`. Jest suites (whatsappRouting, whatsappPipeline) point it at an empty temp dir so unit tests can NEVER load the developer's real session or launch Chrome. Default app behavior is unchanged; do not remove the override.
- catalog_jobs schema completeness (bug P1-09, 2026-08-25): ALL columns the OCR pipeline uses (original_filename, extracted_data, mapping_config, data_filters, error_log, progress, total_count, processed_count, new_count, existing_count, duplicate_count, matched_previous_job_id, newly_detected_columns) are registered in ensureSchema's guarded alterStatements list — the bare CREATE TABLE predates the pipeline. Any NEW catalog_jobs column must be added THERE (idempotent per-boot ALTER), never only to the CREATE block, or upgrades/fresh installs crash with "no such column".
