...
- **GET /inventory/catalog-search** collapses result rows by normalized name (trim/collapsed-spaces/lowercase; in-stock row wins, then lowest id) AFTER stock enrichment — the 18 legacy duplicate-name groups in `medicines` never render twice. Consumers: Purchases page + StagedReviewModal only. The short-query seed branch returns 150 rows for the frontend module-cache pre-hydration.
- **GET /purchases/medicine-batches** expands resolved ids to sibling ids sharing the exact normalized name (`expandSiblingIds`, one indexed equality query) BEFORE querying purchase_items/inventory_master, so batches/rates saved under either twin surface regardless of which id the user picked. Keep this expansion when editing the handler.
