...
- **GET /inventory/catalog-search** collapses result rows by ALPHANUMERIC-STRIPPED name key (`lower(name) minus all non-alphanumerics`; in-stock row wins, then lowest id) AFTER stock enrichment — this hides both the 18 exact duplicate-name groups AND ~493 punctuation/spacing-variant twins ("DOLO-650" vs "DOLO 650") found in the 2026-08 master-DB audit. Consumers: Purchases page + StagedReviewModal only. The short-query seed branch returns 150 rows for the frontend module-cache pre-hydration.
