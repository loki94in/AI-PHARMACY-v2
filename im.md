...
# Target Hardware Baseline: Intel Core i3 3rd-Gen (2 Cores / 4 Threads, HDD, No AVX2)
# Target Runtime Footprint: 0% Idle Process CPU Churn, <140MB RAM Idle Base
# Latency Mandate: <5ms Page Load & Modal Open, <15ms Transaction Sync
# Verification Strictness: 100% Type-Safe, No-Emit Clean, Anti-Regression Guarded
# ==============================================================================

================================================================================
1. COMPREHENSIVE STATUS SUMMARY: CURRENT VS. TARGET BEHAVIORS
================================================================================

This blueprint covers three distinct architectural issues that currently degrade performance on slow, HDD-backed, low-thread-count machines.

--------------------------------------------------------------------------------
1.1 Background Timer Silence & Configuration Gating
--------------------------------------------------------------------------------
*   CURRENT BEHAVIOR:
    - At boot, the main server process or worker supervisor unconditionally spawns background setInterval loops and cron schedules for Gmail IMAP polling, WhatsApp message queues, and Telegram bots.
    - If the store has not configured these features, the console continuously spams uncaught connection exceptions, blocks Node's single thread, and thrashes the mechanical HDD with error log writes.
    - General Settings saves blindly write every form field back to `app_settings`. If a credential field (like `gmail_pass` or Pharmarack password keys) is left empty in the Settings form, it destructively wipes out active configurations set up on the Learning page.
*   TARGET BEHAVIOR:
    - Centralized configuration gates are placed at the entry point of every background loop.
    - If Gmail credentials (`gmail_user`, `gmail_pass`), WhatsApp Web (`whatsapp_enabled`), or Telegram tokens (`telegram_token`) are missing or disabled, their respective background timers instantly return, maintaining 100% silent background CPU usage.
    - All general Settings saves are structurally insulated. Stale form values are filtered out on save to prevent clobbering active credentials. The environment `SKIP_AUTH=true` bypass is preserved cleanly for testing.

--------------------------------------------------------------------------------
1.2 Visible-Screen-Only Math & Incremental Stock Ledger
--------------------------------------------------------------------------------
*   CURRENT BEHAVIOR:
    - Swapping tabs or opening pages like Inventory, Sells, or Reports triggers heavy database queries joining medicines and batches, recalculating loose quantities and active batch flags on the fly.
    - Entering a manual Purchase Bill or typing inside the POS cart triggers hundreds of raw `parseFloat` tax, subtotal, and profit margin recalculations on *every single frame* the user types, causing severe input lag on dual-core CPUs.
    - Historical sales search, non-moving stock queries, and expiry reports run unbounded scans across thousands of transaction rows.
*   TARGET BEHAVIOR:
    - The Inventory page functions as a fast, read-only table. Real stock values are updated transactionally via simple, atomic addition and subtraction during checkout or purchase saves. No full table scans are run on page load.
    - Calculations inside POS and Purchases are strictly memoized using React `useMemo` hooks. The frontend calculations are tied exclusively to the active items currently visible on the screen.
    - Sells, Reports, and Purchase History pages load instantly in <5ms by querying pre-saved, static invoice totals (`total_amount`, `tax_amount`, `net_profit`) written directly to the table columns at transaction save time.

--------------------------------------------------------------------------------
1.3 Idempotent Schema Versioning, Self-Healing Boot, & Process Guardians
--------------------------------------------------------------------------------
*   CURRENT BEHAVIOR:
    - The schema initialization uses over 60 individual `ALTER TABLE` statements wrapped in loose `try/catch` blocks. Every time the app boots, it tries to re-run every alteration, throwing silent SQLITE_ERROR warnings if the columns already exist.
    - Some ALTER statements are written before the corresponding CREATE TABLE is executed, causing latent crashes on fresh installations.
    - Background async loops (like IMAP polling, message queues, and token refreshers) have no `.catch()` handlers. If a network socket drops, an unhandled rejection crashes the entire Express process, taking the pharmacy offline.
*   TARGET BEHAVIOR:
    - A dedicated `schema_migrations` table tracks database evolution. Schema alterations execute sequentially, exactly once, and are fully auditable.
    - A startup database manager executes `PRAGMA integrity_check;` before mounting Express routes. If corruption is detected (e.g., from an abrupt power failure on an HDD), the connection manager renames the corrupt file and automatically restores the latest clean nightly compressed backup in seconds.
    - Process Guardians catch global unhandled rejections, logging them safely while keeping the core server thread running.
...
1.  `src/database.ts` — Define the sequential schema versioning, bump the schema version, and enforce correct CREATE-before-ALTER table initialization.
2.  `src/database/connection.ts` — Implement the startup `PRAGMA integrity_check`, connection timeout tuning, and zlib-compressed self-healing restore loops.
3.  `src/server.ts` — Register global Node.js `uncaughtException` process handlers and implement a `/api/health/ready` check.
4.  `src/worker/emailPoller.ts` — Implement the IMAP poller config-gate check.
5.  `src/services/whatsappQueue.ts` — Implement the `whatsapp_enabled` setting gater.
6.  `src/services/telegramPrescriptionService.ts` — Gate bot connection attempts based on the `telegram_enabled` setting.
7.  `src/routes/sales.ts` — Implement atomic, single-pass transactional stock decrements, log events to `stock_ledger`, and write static invoice totals.
8.  `src/routes/purchases.ts` — Implement atomic, single-pass transactional stock increments and write static purchase totals.
9.  `src/routes/settings.ts` — Secure general settings writes; fix the write path to save into `app_settings` instead of the phantom `settings` table.

### 2.2 Frontend Files to Modify (9 Files)
1.  `frontend/src/pages/POS/index.tsx` — Memoize cart item and tax calculations using `useMemo`, prevent calculations from running on frame renders, and preserve test auth bypass.
2.  `frontend/src/pages/Purchases/index.tsx` — Memoize purchase subtotal and profit margin math for manual invoice entry.
3.  `frontend/src/pages/Inventory/index.tsx` — Configure table as a fast, read-only data view using `@tanstack/react-virtual` with zero client-side stock aggregation.
4.  `frontend/src/pages/Sells/index.tsx` — Direct rendering from pre-saved static invoice totals.
5.  `frontend/src/pages/PurchaseHistory/index.tsx` — Direct rendering from pre-saved static purchase totals.
6.  `frontend/src/pages/Reports/index.tsx` — Read pre-calculated metrics compiled at write-time, staggering heavy report cards.
7.  `frontend/src/pages/Learning/index.tsx` — Display live background service connectivity state badges.
8.  `frontend/src/pages/Settings/index.tsx` — Protect credentials by omitting empty password state variables during general configuration saves.
9.  `frontend/src/components/Layout.tsx` — Render top-level ready-banners that hook into `/api/health/ready` to block UI interactions until the database is fully booted.
...
- Current Behavior: 60+ unversioned schema modification lines thrashed through try/catch blocks on every server startup.
- Target Behavior: Sequential, versioned execution tracked via a `schema_migrations` table to ensure ALTER statements run exactly once.
```typescript
...
import { getAppDataDir } from '../config/index.js'; // Respect packaged AppData dir contract
...
class DatabaseManager {
  ...
  public getConnection(): Database {
    ...
    const dbPath = path.join(getAppDataDir(), 'data', 'app.db');
    const backupDir = path.join(getAppDataDir(), 'backup');

    try {
      this.connection = new DatabaseConstructor(dbPath, { fileMustExist: false });
      
      // Enforce high-performance WAL configuration to mitigate HDD disk bottlenecks
      this.connection.pragma('journal_mode = WAL');
      this.connection.pragma('busy_timeout = 30000'); // Mitigate cross-process write locks
      this.connection.pragma('synchronous = NORMAL');

      // Run boot integrity validation
  ...
  private runSelfHealing(dbPath: string, backupDir: string) {
    ...
    // Locate the newest compressed zlib backup
...
import compression from 'compression'; // Enable Gzip for high-speed local packet transfers
...
// Process Guardians to intercept uncaught async loops and keep the server online
...
router.post('/invoice', (req, res) => {
  ...
  try {
    db.transaction(() => {
      ...
      // Calculate totals once server-side
      const totalAmount = items.reduce((acc: number, item: any) => acc + (item.rate * item.qty), 0);
      const taxAmount = totalAmount * 0.05; // Enforce 5% Flat Tax Contract
      const netProfit = totalAmount - (totalAmount * 0.70); // 70% COGS calculation

      // 1. Save static totals permanently inside the invoice header
      db.prepare(`
      ...
      // 2. Process stock adjustments atomically per item
      for (const item of items) {
        // Strict quantity decrement - prevents negative values or overselling
        ...
        // Write append-only entry to the stock_ledger audit table
        db.prepare(`
...
// Fetch pre-saved computed totals for sales history lists in <2ms
...
router.post('/manual', (req, res) => {
  ...
  try {
    db.transaction(() => {
      // 1. Calculate static totals once
      const subtotal = items.reduce((acc: number, item: any) => acc + (item.rate * item.qty), 0);
      const taxTotal = subtotal * 0.12; // Standard 12% GST fallback
      ...
      // 2. Increment stock ledger atomically
      for (const item of items) {
        db.prepare(`
...
// Protect active credentials from being cleared or blanked out by destructive saves
  ...
  try {
    db.transaction(() => {
      for (const [key, value] of Object.entries(payload)) {
        // Enforce write path correctness: Use app_settings, never the incorrect settings table
        ...
        if (PROTECTED_CRED_KEYS.includes(key) && (!value || String(value).trim() === '')) {
          continue; // Maintain existing credentials; skip blank configurations
        }
...
export function POSBillingEngine() {
  ...
  const handleCompleteSale = () => {
    // Phone Gater Check: Skip phone requirement for cash/walk-ins when WhatsApp is off
...
export function VirtualizedInventoryTable({ inventoryItems }: { inventoryItems: any[] }) {
  ...
  // Virtualize the list to render only the visible rows
...
To maintain complete system stability, the new event-driven precalculated cache runs alongside the following isolated pipeline gates:
   ...
   (Pre-check schema & integrity)           (Review tax & cost changes)
     ...
     [Final Approve Trigger] ──► [BEGIN IMMEDIATE DB TRANSACTION]
                                   │
                                   ├──► UPDATE inventory_master (Single Write)
                                   ├──► INSERT stock_ledger (Audit Write)
                                   └──► COMMIT ──► [Server-Sent Events Broadcast]
...
Finalizing the migration executes the schema cutover routine. This routine calculates first-time stock ledger totals and writes them directly into inventory_master. Only after this step is complete does the application transition to incremental/decremental ledger changes.
Purchase Invoice Auto-Extraction:
OCR-scanned invoices or email attachments resolve medicine name aliases cleanly.
Staged data is rendered inside the manual Purchases page preview. Upon approval, the saving transaction increments physical stock in inventory_master transactionally.
