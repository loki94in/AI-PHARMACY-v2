  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-3 mb-6">
        <div>
          <h2 className="text-[26px] sm:text-[32px] font-extrabold tracking-tight mb-1 text-text">Welcome back, Admin</h2>
          <p className="text-muted text-base sm:text-lg">Here's what's happening at your pharmacy today.</p>
        </div>
        <div className="text-base font-semibold text-sky bg-sky/10 px-4 py-2 rounded-full border border-sky/20 self-start sm:self-auto">
          Today: {dateStr}
        </div>
      </div>
      ...
      <div className="glass-panel p-4 mb-6">
        <div className="text-sm font-bold text-muted uppercase tracking-wider mb-3">Quick Actions</div>
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          <Link to="/pos" className="flex items-center gap-2.5 p-3 rounded-xl bg-primary/10 border border-primary/20 text-primary hover:bg-primary/20 transition-all font-semibold text-base">
            <ShoppingCart size={19} />
            <span>New POS Bill</span>
          </Link>
          <Link to="/purchases" className="flex items-center gap-2.5 p-3 rounded-xl bg-sky/10 border border-sky/20 text-sky hover:bg-sky/20 transition-all font-semibold text-base">
            <PlusCircle size={19} />
            <span>Add Purchase</span>
          </Link>
          <Link to="/inventory" className="flex items-center gap-2.5 p-3 rounded-xl bg-amber/10 border border-amber/20 text-amber hover:bg-amber/20 transition-all font-semibold text-base">
            <Search size={19} />
            <span>Inventory</span>
          </Link>
          <Link to="/crm?tab=special_orders" className="flex items-center gap-2.5 p-3 rounded-xl bg-purple-500/10 border border-purple-500/20 text-purple-400 hover:bg-purple-500/20 transition-all font-semibold text-base">
            <ClipboardList size={19} />
            <span>Special Orders</span>
          </Link>
          <Link to="/crm" className="flex items-center gap-2.5 p-3 rounded-xl bg-bg3 border border-border text-text hover:bg-bg2 transition-all font-semibold text-base">
            <Users size={19} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6">
        ...
        <div className="glass-panel p-6 relative overflow-hidden group">
          ...
          <div className="text-sm text-muted font-bold uppercase tracking-wider mb-2">Today's Sales</div>
          <div className="text-[32px] font-extrabold text-green mb-3">
        ...
        <div className="glass-panel p-6 relative overflow-hidden group">
          ...
          <div className="text-sm text-muted font-bold uppercase tracking-wider mb-2">Low Stock Items</div>
          <div className="text-[32px] font-extrabold text-red mb-3">
            {stats?.lowStock || 0}
          </div>
        ...
        <div className="glass-panel p-6 relative overflow-hidden group">
          ...
          <div className="text-sm text-muted font-bold uppercase tracking-wider mb-2">Pending Alerts</div>
          <div className="text-[32px] font-extrabold text-amber mb-3">
        ...
        <div className="glass-panel p-6 relative overflow-hidden group">
          ...
          <div className="text-sm text-muted font-bold uppercase tracking-wider mb-2">System Status</div>
          <div className="text-[26px] font-bold text-sky mb-3 mt-1 flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <div className="glass-panel p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-xs font-bold text-muted uppercase tracking-wider">Storage Racks</div>
            <div className="text-xl font-bold text-text">{stats?.storageLocations || 5} active</div>
          </div>
        </div>
        ...
        <div className="glass-panel p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-xs font-bold text-muted uppercase tracking-wider">Special Orders</div>
            <div className="text-xl font-bold text-amber">{stats?.pendingSpecialOrders || 0} pending</div>
          </div>
        </div>
        ...
        <div className="glass-panel p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-xs font-bold text-muted uppercase tracking-wider">Delivery Boys</div>
            <div className="text-xl font-bold text-sky">{stats?.activeDeliveryBoys || 0} staff</div>
          </div>
        </div>
        ...
        <div className="glass-panel p-4 flex items-center gap-3">
          ...
          <div>
            <div className="text-xs font-bold text-muted uppercase tracking-wider">Today Purchases</div>
            <div className="text-xl font-bold text-text">₹{Number(stats?.todayPurchases || 0).toLocaleString('en-IN', { maximumFractionDigits: 0 })}</div>
          </div>
        </div>
      </div>
      ...
      {stats?.alerts && stats.alerts.length > 0 && (
        <div className="glass-panel border-amber-500/20 bg-amber-500/5 overflow-hidden mb-6">
          <div className="p-5 border-b border-amber-500/20 flex justify-between items-center bg-amber-500/10">
            ...
            <span className="text-xs font-bold bg-amber-500/20 border border-amber-500/30 text-amber-500 px-2 py-0.5 rounded-full uppercase">
            ...
            {stats.alerts.slice(0, 10).map(alert => (
              <div key={alert.id} className="p-4 flex items-center justify-between gap-4 hover:bg-bg3/50 transition-all">
                <div className="flex flex-col gap-1">
                  <p className="text-base font-semibold text-text">{alert.description}</p>
                ...
                <button
                  ...
                  className="px-3 py-1.5 min-h-8 bg-bg2 hover:bg-bg3 text-muted hover:text-text text-[13px] font-bold border border-border rounded-lg transition-all cursor-pointer"
                >
                  Dismiss
                </button>
              </div>
      ...
      <div className="glass-panel overflow-hidden mb-6">
        <div className="p-5 border-b border-border flex justify-between items-center bg-bg2/40">
          <h3 className="text-lg font-bold flex items-center gap-2 text-text">
            <Activity size={19} className="text-amber" /> 
            Recent Sales Activity
          </h3>
          <Link to="/sells" className="text-sm font-semibold text-primary hover:underline flex items-center gap-1">
        ...
        <div className="overflow-x-auto">
          {hasRecentSales ? (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-border bg-bg3/50 text-muted text-sm uppercase font-bold tracking-wider">
              ...
              <tbody className="divide-y divide-border text-base">
                {stats.recentSales!.map((sale) => (
                  <tr key={sale.id} className="hover:bg-bg3/30 transition-all">
                    ...
                    <td className="p-4">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-sm font-bold bg-bg3 text-text border border-border">
            ...
            <div className="p-8 text-center space-y-3">
              <p className="text-muted text-base">No recent sales recorded yet today.</p>
              <Link to="/pos" className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-xl text-base font-bold shadow hover:opacity-90 transition-all">
                <ShoppingCart size={17} /> Create First POS Sale
              </Link>
            </div>
          )}
        </div>
      ...
      <div className="glass-panel border-sky/20 bg-sky/5 overflow-hidden">
        <div className="p-5 border-b border-sky/20 flex justify-between items-center bg-sky/10">
          <h3 className="text-lg font-bold flex items-center gap-2 text-sky">
            <MessageCircle size={19} /> 
            Recent Communications & Ingestion
          </h3>
          <Link to="/mail" className="text-sm font-semibold text-sky hover:underline flex items-center gap-1">
            Open Mail <ArrowRight size={15} />
          </Link>
        </div>
        <div>
          {hasRecentComms ? (
              ...
              {stats.recentCommunications!.map((comm, idx) => (
                <div key={idx} className="p-4 flex items-center justify-between gap-4 hover:bg-bg3/50 transition-all">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-base font-semibold text-text">{comm.title || 'Email Ingestion'}</p>
                      <p className="text-sm text-muted">{comm.recipient_or_sender || 'System'}</p>
                    </div>
                  </div>
                  <span className="text-xs text-muted font-mono">
            ...
            <div className="p-8 text-center text-base text-muted space-y-2">
              <p>No recent email or message logs found.</p>
              <Link to="/mail" className="inline-flex items-center gap-1.5 text-sm text-sky font-semibold hover:underline">
                <Mail size={15} /> Check Inbox
              </Link>
            </div>
          )}
        </div>
