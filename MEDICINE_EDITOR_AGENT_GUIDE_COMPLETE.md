# MEDICINE EDITOR — AGENT IMPLEMENTATION GUIDE (COMPLETE)
...
**Version:** 2.0 (with leave-out list and bug fixes)
...
10. [Leave-Out List](#leave-out-list)
...
### Step 2.4: Fix pgMasterImporter Bug

**File:** `src/worker/importers/pgMasterImporter.ts`

**Action:** Find the row mapping where `row['therapeutic']` is incorrectly mapped to `schedule_type`. Fix it:

```typescript
// WRONG (current):
schedule_type: row['therapeutic'] || null,

// CORRECT:
schedule_type: row['schedule_type'] || null,
therapeutic: row['therapeutic'] || null,
```

**Verification:**
- [ ] Read the file to confirm the bug is fixed
- [ ] Verify `therapeutic` maps to `therapeutic` column
- [ ] Verify `schedule_type` maps to `schedule_type` column

### Step 2.5: Update catalogWorker

**File:** `src/worker/catalogWorker.ts`

**Action:** Add new field mappings for CSV import:

```typescript
therapeutic: row['therapeutic'] || null,
sub_therapeutic: row['sub_therapeutic'] || null,
short_code: row['short_code'] || null,
ucode: row['ucode'] || null,
```

**Verification:**
- [ ] Read the file to confirm new field mappings are added

### Step 2.6: Update invoiceService

**File:** `src/services/invoiceService.ts`

**Action:** Add H1 warning check when creating invoices:

```typescript
// Before saving invoice, check for H1 drugs
if (item.schedule_type?.toUpperCase() === 'H1') {
  if (!item.doctor_name) {
    throw new Error(`Doctor name required for H1 drug: ${item.name}`);
  }
}
```
...
## LEAVE-OUT LIST — DO NOT MODIFY

### Safe-Zone Files (NEVER touch)
- [ ] `src/services/stockRebuild.ts` — stock recalculation logic
- [ ] `src/services/pdfInvoiceService.ts` — PDF generation
- [ ] `src/services/pharmarack.ts` — Pharmarack integration
- [ ] `src/services/emailService.ts` — email sending
- [ ] `src/services/whatsappService.ts` — WhatsApp integration
- [ ] `src/services/telegramService.ts` — Telegram integration
- [ ] `src/services/backupService.ts` — backup/restore
- [ ] `src/services/cronScheduler.ts` — scheduled tasks
- [ ] `src/services/activityTracker.ts` — user activity tracking
- [ ] `src/services/userService.ts` — user authentication
- [ ] `src/services/smsService.ts` — SMS sending
- [ ] `src/services/exportService.ts` — data export

### Protected DB Tables (NEVER alter schema)
- [ ] `users` — user accounts
- [ ] `audit_log` — audit trail
- [ ] `stock_rebuild_log` — stock rebuild history
- [ ] `sales` — sales transactions
- [ ] `sale_items` — sale line items
- [ ] `purchase_invoices` — purchase invoices
- [ ] `pharmarack_sync_log` — Pharmarack sync history
- [ ] `whatsapp_log` — WhatsApp message log
- [ ] `telegram_log` — Telegram message log

### Out-of-Scope Features (DO NOT change behavior)
- [ ] Invoice PDF generation format
- [ ] Pharmarack cart sync logic
...
- [ ] Export/import of sales data
- [ ] Stock rebuild calculation

### UI Patterns to Preserve (DO NOT change)
- [ ] Centered modal layout
- [ ] Structured name builder
- [ ] Single scrollable form (no tabs/accordion)
- [ ] Existing color scheme (semantic Tailwind variables)
- [ ] Existing button styles
- [ ] Existing form validation patterns
- [ ] Existing toast notification system

### Existing Features to Keep Working (DO NOT break)
- [ ] Stock management (add/edit/delete)
- [ ] Purchase invoices (create/edit)
- [ ] Sales history (view/search)
- [ ] CRM (customer management)
- [ ] Dispatch (delivery boy management)
- [ ] Learning (OCR corrections)
- [ ] Settings (store configuration)
- [ ] Reports (sales/stock reports)

### Integrations to Not Break (DO NOT disrupt)
- [ ] Pharmarack API sync
